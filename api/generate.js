...
【手書きの書き込みは信じないこと】
この写真は、子供が答えを書き込んだあとの問題集であることがあります。
鉛筆やペンで手書きされた文字は、答え合わせをしていない可能性が高く、
まちがっていることもあります。したがって:
- 手書きの書き込みを、正解の根拠にしてはいけない。
- source（引用）に手書きの文字を写してはいけない。印刷された文だけを写す。
- 印刷された本文から答えが確定できる問題だけを作る。
- 選択肢から答えを絞りこむ場合は、あてはまるものが1つしかないと
  はっきり言いきれるときだけにする。言いきれないなら、その問題は作らない。

【絶対に守ること】
1. 画像に印刷されている内容だけから作る。知識で補わない。書いていないことは出題しない。
...
F. 根拠が、教材に印刷されている文である。答えを書き込んだ手書きの文字を
   写しただけに見えるものはNG（手書きは間違っている可能性があるため）。
...
出力はJSONのみ（説明文・マークダウン不要）：
{
 "themes":[
   {"title":"短い見出し(例:有明海)","key":"覚える1語",
    ...
    "kaisetsu":"基本解説(2〜3文。小3が一読してわかる、やさしい言葉。1文は短く。専門語を使うときは（◯◯ということ）のように一言でかみ砕く。むずかしい漢字には(ふりがな))",
    "deep":"もっとくわしく(3〜4文。中学受験で差がつく深い内容：背景・因果・関連する地名や人物・数値・他単元とのつながり。具体的に。ここは難度が高くてもよい)",
...
- kaisetsu は必ず小3が一度読んで理解できるやさしさにする。deepは難度が高くてよい（基本と深掘りの差を大きくつける）。
- 文章は必ず確定した内容だけを書く。「いや」「ではなく」「？」のような書き直し・迷いの跡を地の文に残さない（候補を並べて悩んだ様子を見せない）。
...
- セリフ・見出しは確定した内容だけを書く。「いや」「ではなく」「？」のような書き直し・迷いの跡を残さない
