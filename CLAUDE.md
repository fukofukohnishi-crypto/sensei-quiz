...
`PREF_DATA` に県コードをキーとして持つ。**現在27県を実装**:

- `"31"`〜`"47"` 中国・四国・九州沖縄（17県、カードは `r063`〜`r079`）
- `"20"`〜`"30"`（22をのぞく10府県）近畿と中部（カードは `r080`〜`r089`）。
  10月の社会テキスト「ぐるっと日本ひとめぐり」に出てくる府県で、
  **クイズ100問はすべてそのテキストの内容から作ってある**。
  ニッポン制覇がそのままマンスリー対策になる。

のこり20道県は未実装で、タップすると「じゅんびちゅう！」と出る。

**`"20"`〜`"30"` の絵はまだ無い**（立ち絵10・メダル100・カード10）。
立ち絵は絵文字に、メダルは非表示に、カード絵は `cardImgFB` で落ちるので
画面は壊れない。絵ができたら差しかえるだけでよい。
