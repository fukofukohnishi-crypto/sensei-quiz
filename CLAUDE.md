...
| 値 | 枚数 | 中身 | 入手 |
|---|---|---|---|
| `gakuen` | 54 | オリジナル学園の先生たち | ガチャ／10問クリア |
...
- **`challenge` シリーズのカードはガチャに出してはいけない。** 50問連続クリアの
  ごほうびとしての意味がなくなるため。`october.html` の `pickCard()` は
  `isSpecial()` で外している。凍結ずみの `may.html` と `natsuyasumi.html` も、
  抽選プールを `series!=='challenge'` で絞るように直してある（10月版のカードが
  Cards.json に入ったことで、古いアプリのガチャから出てしまうため）。
