...
Cards.json        カード65枚のマスタデータ
stories.json      学園の読み物23話。10月版のご褒美（連続正解のベスト更新で1話解放）
...
| 値 | 枚数 | 中身 | 入手 |
|---|---|---|---|
| `gakuen` | 45 | オリジナル学園の先生たち | ガチャ／10問クリア |
...
- 連続正解の**自己ベストを更新するたび**に `stories.json` が1話ずつ解放される。
  いま23話。**ベストを23回更新すると打ち止め**になるので、そうなる前に足すこと
  （`unlockNext()` は在庫切れでも落ちない）。
