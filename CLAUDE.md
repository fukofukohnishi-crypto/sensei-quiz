...
**「しくみ」の説明は `ansFig` ではなく `figAfter` にする。** `ansFig` は高さ170pxで
出るので、**ラベルのある図を入れても字が読めない**（輪中の地図を入れようとして
実際にそうなった）。`figAfter` は幅いっぱい・高さ360pxまで使えるので、
ラベル付きの図はこちらに置く。`ansFig` は「実物が1つ写っているもの」専用。
...
**高さ170pxだけを決めて、横は絵にまかせている**ので、写真は縦横どんな比でも
そのまま入れられる。角は `border-radius:14px` でまるめる。自作SVGは正方形
なので、これまでどおり170pxの丸に見える。
