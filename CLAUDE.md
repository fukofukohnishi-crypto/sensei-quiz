...
- **ランダムに出していいのは `gakuen` シリーズだけ。** 抽選プールは
  ...
  カードを足すたびに同じ穴が空く。
- 読み物の解放は **`STORY_MILESTONES`（5・10・15…45）をベストがこえたとき** と
  **50問クリアするたび `CLEAR_STORIES`（2話）**の2つ。カードの絵をたくさん
  用意できないぶん、**読み物が挑戦の燃料、レアカードがゴール**という配分にしてある。
  - 1科目で50まで登ると 節目9＋クリア2＝**11話**。2科目で22話、＋最終話で23話。
    のこり24話は**クリア1回につき2話**なので、あと12回。
  - `CLEAR_STORIES` は最初1話だった。ベストが50に達すると節目が残らず、
    読み物が進む手段がクリアだけになる。50問連続は1問ミスでその場終了なので
    （自己採点の正答率95%でも1回の成功率は8%ほど）、26回クリアは重すぎた。
...
**最終話だけは順番では開かない。** `october.html` の `finalStoryId()` が
