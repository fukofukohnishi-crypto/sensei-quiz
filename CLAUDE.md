...
- **読み物は「れんしゅう」のごほうび、レアカードは「50問連続チャレンジ」のごほうび。**
  ...
  - れんしゅう（`mode==='free'`）とまちがい直し（`'review'`）で
    **`STORY_EVERY`（10問）こたえるごとに1話**。れんしゅうは1回20問なので1回で2話。
    数えているのは `practiceCount`（`oct_practice_v1`）で、**チャレンジの解答は数えない**。
  - **50問連続チャレンジでは読み物は開かない。** あちらは `sp04`/`sp05`/`sp06` を取りにいく場所。
  - 最終話だけは例外で、融合カード `sp06` を取ったときに開く（下記）。
  - 目安: れんしゅう1回で2話、5回で10話、**23回（460問）で最終話をのぞく46話**。
    問題バンクは514問なので、ひととおり解くと読みきる配分。
