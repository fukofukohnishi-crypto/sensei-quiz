...
| `pref` | 10 | 都道府県の守護神（`r080`〜`r089`） | ニッポン制覇でメダル10個 |
...
たとえ正しくても入れない。テキストが手元にない県は**実装しない**。
...
`PREF_DATA` に県コードをキーとして持つ。**いまは10府県だけ**。メダルは100個。

| コード | 地方 | カード | 教材 |
|---|---|---|---|
| `"20"`〜`"30"`（22をのぞく） | 近畿・中部の10府県 | `r080`〜`r089` | テキスト第1〜5回から **100/100個**。絵まち |

のこり37都道府県は**教材が手元にないので実装しない**。タップすると
「じゅんびちゅう！」と出る。テキストの写真が来たら、その回のぶんだけ足す。

一度は47都道府県ぜんぶ入れたが、教材外のクイズで埋めていたため取りのぞいた。
git に残っているので、テキストが来たら骨組みとして使える。

- `247aaea` … 北海道・東北・関東・北陸ほか20道県（`r090`〜`r109`）
- `e81011a` … 中国・四国・九州沖縄の17県（`r063`〜`r079`）

**外した県の絵は消していない。** `chars/` に193ファイル、`card-images/` に19ファイル
残っている。県がもどってくればそのまま使える。
...
**メダル100個の絵はそろっている**（`chars/medals/kinki/` と `chubu/`、
WebP q85・252px・1枚17KBほど・合計1.7MB）。まだ無いのは**守護神の立ち絵10枚**
（`chars/portraits/*.webp`）と**守護神カードの絵10枚**（`r080`〜`r089`）だけ。
無いあいだは立ち絵が絵文字に、カード絵は `cardImgFB` で落ちるので画面は壊れない。
守護神の名前も仮なので、絵にあわせて変えてよい。
