...
**写真は `tools/photo2circle.py` でまるく切り抜く。** SVGの絵と見た目をそろえるため。

```bash
python3 tools/photo2circle.py <写真> <名前> [x0 y0 x1 y1]   # 切り抜く範囲は省略可
```

440px・WebP q85・まるいアルファ付き（1枚25KBほど）。正方形でない写真は、
足りない側を元の絵をぼかして引きのばしたもので埋めるので、背景がつながって見える。
**被写体が円いっぱいになるように範囲を決めること。** 全身を入れようとすると
小さくなりすぎる（表示は170px）。見分けのつく部分を優先する。
