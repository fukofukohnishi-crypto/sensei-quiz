...
`Cards.json` が唯一のマスタ。100枚。1件はこの形:
...
| 値 | 枚数 | 中身 | 入手 |
|---|---|---|---|
| `gakuen` | 50 | オリジナル学園の先生たち | ガチャ／10問クリア |
| `pref` | 47 | 都道府県の守護神（`r063`〜`r109`） | ニッポン制覇でメダル10個 |
...
`PREF_DATA` に県コード（`"01"`〜`"47"`）をキーとして持つ。
**47都道府県すべて実装ずみ**。メダルは合計470個。

| コード | 地方 | カード | 備考 |
|---|---|---|---|
| `"31"`〜`"47"` | 中国・四国・九州沖縄 | `r063`〜`r079` | 絵もそろっている |
| `"20"`〜`"30"`（22をのぞく） | 近畿・中部の10府県 | `r080`〜`r089` | **絵まち** |
| `"01"`〜`"19"`・`"22"` | 北海道・東北・関東・北陸ほか | `r090`〜`r109` | **絵まち** |

`"20"`〜`"30"` のクイズ100問は、**10月の社会テキスト
「ぐるっと日本ひとめぐり」の内容だけ**から作ってある。
ニッポン制覇がそのままマンスリー対策になる。
