...
`PREF_DATA` に県コードをキーとして持つ。**現在27県**。メダルは270個。

| コード | 地方 | カード | 教材 |
|---|---|---|---|
| `"20"`〜`"30"`（22をのぞく） | 近畿・中部の10府県 | `r080`〜`r089` | **テキスト第1〜5回から100/100個**。絵まち |
| `"31"`〜`"47"` | 中国・四国・九州沖縄 | `r063`〜`r079` | **教材外が多い（14/170個しか一致しない）。要差しかえ** |

のこり20道県（`"01"`〜`"19"`・`"22"`）は、**教材が手元にないので実装しない**。
一度クイズを書いて入れたが、一般知識で作ってしまったため取りのぞいた
（git の `247aaea` に残っている。テキストが来たら骨組みとして使える）。

**絵がまだ無い10府県ぶんは、立ち絵は絵文字に、メダルは非表示に、
カード絵は `cardImgFB` で落ちる**ので画面は壊れない。差しかえるだけでよい。
守護神の名前も仮なので、絵にあわせて変えてよい。

**絵がまだ無い30県ぶんは、立ち絵は絵文字に、メダルは非表示に、
カード絵は `cardImgFB` で落ちる**ので画面は壊れない。差しかえるだけでよい。
守護神の名前も仮なので、絵にあわせて変えてよい。
